  ...
  try {
    ...
    if (products.length > 0) {
      ...
      products.forEach(product => {
        console.log(`- ${product.title}`);
        console.log(`  Brand: ${product.brand_name}`);
        console.log(`  Price: $${product.offers?.[0]?.price?.price || 'N/A'}`);
...
<Check>
  **Available Filters:**
  ...
  - `availability`: Stock status ("InStock", "OutOfStock", "PreOrder", "LimitedAvailability", "BackOrder", "Discontinued", "SoldOut", "Unknown")
...
async function getProductDetail(productId: string) {
  try {
    ...
    // Pricing information
    if (productDetail.offers && productDetail.offers.length > 0) {
      console.log(`\nPricing:`);
      productDetail.offers.forEach((offer, index) => {
        console.log(`  Offer ${index + 1}: $${offer.price.price} ${offer.price.currency}`);
        console.log(`    Seller: ${offer.seller_name}`);
...
class ProductSearchApp {
  ...
  private displayProduct(product: Product, index: number): void {
    ...
    if (product.offers && product.offers.length > 0) {
      ...
      console.log(`   Price: $${offer.price.price} ${offer.price.currency}`);
      console.log(`   Seller: ${offer.seller_name}`);
      console.log(`   Stock: ${offer.availability}`);
    }
  ...
  async runDemo(): Promise<void> {
    ...
    // 4. Error handling demo (intentional error)
